...
public:
    ...
    int init() {
        event_driver_ = ev_default_loop (0);
        if (!event_driver_) {
            return -1;
        }
...
int SchedMgr::create(easyco_coroutine_t* c, const std::SchedMgr::func_type& func)
{
...
void SchedMgr::resume(easyco_coroutine_t* c)
{
    auto task = this.sched_task[c.taskid];
    task.resume(c);
}

void SchedMgr::yield(easyco_coroutine_t* c)
{
    auto task = this.sched_task[c.taskid];
    task.yield(c);
}
